...
from utils.json_read_write import get_value_from_section, get_config_path, write_value_to_section
import hashlib
...
def erstelle_ordner():
    ...
    try:
        if aktueller_nachtordner is None or not os.path.exists(aktueller_nachtordner):
            ...
            write_value_to_section(get_config_path("Lepmon_config.json"), "general", "current_folder",aktueller_nachtordner)
...
def initialisiere_logfile():
  aktueller_nachtordner = get_value_from_section(get_config_path("Lepmon_config.json"),"general","current_folder")
  #jetzt_local = datetime.now()
  ...
  try:
        ...
        if not os.path.exists(log_dateipfad):
            with open(log_dateipfad, 'w') as f:
                ...
                write_value_to_section(get_config_path("Lepmon_config.json"), "general", "current_log",log_dateipfad)
