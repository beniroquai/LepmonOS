...
def main_loop():
    ...
    try:
        ...
        # Store USB space information in FRAM
        try:
            # Write free space as float (4 bytes) to FRAM
            write_fram_bytes(0x0390, struct.pack('f', free_space_gb))
            append_log(f"USB free space ({free_space_gb} GB) stored in FRAM")
        except Exception as fram_error:
            append_log(f"Failed to write USB space to FRAM: {fram_error}")
